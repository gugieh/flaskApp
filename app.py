...
def dashboard():
    ...
    return render_template('dashboard.html', title='Dashboard', users=users, addUser=addUser, editUser=editUser, editUserPass=editUserPass, search=search, createFolder=createFolder, uploadFile=uploadFile, files=files, folders=folders, renameFolder=renameFolder)
...
def renameFolder():
    ...
    if id:
        folder_path = os.path.join(app.config['UPLOAD_PATH'], id)
        folder_end = os.path.join(app.config['UPLOAD_PATH'], folderName)
        folder = Folders.query.filter_by(folderName=id).one()
        if folder:
            folder.folderName = folderName
            db.session.commit()
            if os.path.exists(folder_path):
                try:
                    os.rename(folder_path, folder_end)
                    print(f'Katalog {folder_path} został usunięty')
                except OSError:
                    print('Nie można usunąć katalogu, ponieważ nie jest pusty')
            else:
                print('Podany katalog nie istnieje')
        else:
            print('Folder o podanym id nie istnieje w bazie danych')
